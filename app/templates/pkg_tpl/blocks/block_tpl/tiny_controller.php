...
class <%=blockhandle%>Tiny
{
  ...
  public function getImportData($blockNode) {
      ...
      $content = preg_replace_callback(
          '/\{ccm:export:page:(.*)\}/i',
          array('<%=blockhandle%>Tiny', 'replacePagePlaceHolderOnImport'),
          $content);

      $content = preg_replace_callback(
          '/\{ccm:export:image:(.*)\}/i',
          array('<%=blockhandle%>Tiny', 'replaceImagePlaceHolderOnImport'),
          $content);

      $content = preg_replace_callback(
          '/\{ccm:export:file:(.*)\}/i',
          array('<%=blockhandle%>Tiny', 'replaceFilePlaceHolderOnImport'),
          $content);

      $content = preg_replace_callback(
          '/\{ccm:export:define:(.*)\}/i',
          array('<%=blockhandle%>Tiny', 'replaceDefineOnImport'),
          $content);
  ...
  function translateFromEditMode($text) {
      ...
      $text = preg_replace_callback(
          '/{CCM:FID_([0-9]+)}/i',
          array('<%=blockhandle%>Tiny', 'replaceFileIDInEditMode'),
          $text);


      $text = preg_replace_callback(
          '/{CCM:FID_DL_([0-9]+)}/i',
          array('<%=blockhandle%>Tiny', 'replaceDownloadFileIDInEditMode'),
          $text);
  ...
  function translateFrom($text) {
      ...
      $text = preg_replace_callback(
          '/{CCM:CID_([0-9]+)}/i',
          array('<%=blockhandle%>Tiny', 'replaceCollectionID'),
          $text);

      $text = preg_replace_callback(
          '/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
          array('<%=blockhandle%>Tiny', 'replaceImageID'),
          $text);

      // now we add in support for the files that we view inline
      $text = preg_replace_callback(
          '/{CCM:FID_([0-9]+)}/i',
          array('<%=blockhandle%>Tiny', 'replaceFileID'),
          $text);
      ...
      $text = preg_replace_callback(
          '/{CCM:FID_DL_([0-9]+)}/i',
          array('<%=blockhandle%>Tiny', 'replaceDownloadFileID'),
          $text);
